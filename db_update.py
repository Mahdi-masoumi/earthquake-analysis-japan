...
def run_database_final():
    ...
    df_dataset = clean_df(pd.read_csv("JAPAN_DATASET_cleaned.csv"), "Dataset")
    df_emsc = clean_df(pd.read_csv("JAPAN_EMSC_cleaned.csv"), "EMSC")
    df_geofon = clean_df(pd.read_csv("JAPAN_GEOFON_cleaned.csv"), "GEOFON")
    df_usgs = clean_df(pd.read_csv("JAPAN_USGS_cleaned.csv"), "USGS")

    df_final = pd.concat([df_dataset, df_emsc, df_geofon, df_usgs], ignore_index = True).drop_duplicates()

    df_final.to_sql(name = "Earthquakes", 
                    con = engine,
                    if_exists ="replace", 
                    index = False, 
                    chunksize = 1000)
