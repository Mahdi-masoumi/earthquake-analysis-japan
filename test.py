...
class unittest_class(unittest.TestCase):
    ...
    def test_col(self):  # Checking that these columns exist in the file.
        columns_ = ["time", "latitude", "longitude", "mag",
                    "region", "depth", "tokyo_distance", "place"]
        for file in self.files:
            df = pd.read_csv(file)
            for col in columns_:
                self.assertIn(col, df.columns)

    # Checking that the values in these columns are numeric.
    def test_num(self):
        num_columns = ["latitude", "longitude",
                       "depth", "mag", "tokyo_distance"]
        for file in self.files:
            df = pd.read_csv(file)
    ...
    def test_chvalue(self):  # Checking that there are no NaN values
        ch_columns = ["time", "latitude", "longitude", "depth",
                      "mag", "tokyo_distance", "region", "place"]
        for file in self.files:
            df = pd.read_csv(file)
