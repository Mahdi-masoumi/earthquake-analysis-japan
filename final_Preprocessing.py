...
def preprocessing():
    ...
    def missing_values(cleaned_df):
        # drop NaN time, latitude and longitude
        ...
        for column in ["mag", "depth"]:
            if column in cleaned_df.columns:
                ...
                cleaned_df.loc[:, column] = cleaned_df[column].fillna(
                    mean_dm).round(1)
    ...
    def extract_emsc_geofon(place):
        ...
        removing_words = ["off", "near", "western", "eastern", "east", "west",
                          "north", "south", "coast of", "region", "japan", "of", ",", "s."]
    ...
    for f in csv_files:
        ...
        if "coordinates" in cleaned_df.columns:
            ...
            cleaned_df["longitude"] = coordinates[0].str.replace(
                "°E", "").str.strip().astype(float)
            cleaned_df["latitude"] = coordinates[1].str.replace(
                "°N", "").str.strip().astype(float)
        ...
        cleaned_df.loc[(cleaned_df["mag"] < 0) | (
            cleaned_df["mag"] > 10), "mag"] = pd.NA  # 0 < Magnitude < 10

        cleaned_df.loc[(cleaned_df["depth"] < 0) | (
            cleaned_df["depth"] > 700), "depth"] = pd.NA  # 0 < Depth < 700

        cleaned_df.loc[cleaned_df["latitude"] <
                       0, "latitude"] = pd.NA  # latitude +

        cleaned_df.loc[cleaned_df["longitude"] <
                       0, "longitude"] = pd.NA  # longitude +
    ...
    # task8 extract region
        ...
        if f == "JAPAN_USGS.csv":
            ...
            cleaned_df["region"] = cleaned_df["region"].apply(
                extract_emsc_geofon)
        ...
        cleaned_df["tokyo_distance"] = np.sqrt(
            (x - tokyo_long)**2 + (y - tokyo_lat)**2).round(2)
        ...
        print(std_distance)
        percentiles_distance = np.percentile(
            cleaned_df["tokyo_distance"], [25, 50, 75])
        print(percentiles_distance)

        # region (ryukyu islands)
        cleaned_df["region"] = cleaned_df["region"].replace(
            "Ryukyu Isl.", "Ryukyu Islands")
